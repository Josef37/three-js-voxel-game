...
/** Represents a cuboidal (brick-shaped) part of the world. All vertices have integer coordinates and the blocks have size 1. */
export class Chunk {
  /**
  ...
  constructor (chunkSize, worldPosition) {
    ...
    /**
     * The global position of the corner with the smallest coordinates (minimal x,y,z)
     * @type {Vector3}
     */
    this.worldPosition = worldPosition

    /**
     * All block types of this chunk in an flat array.
     * Increases x coordinate first, then y, then z.
     * @type {Uint8Array}
     */
  ...
  computeGeometry () {
    ...
    this.iterateBlocks((x, y, z, index) => {
      if (blockTypes[index] === 0) return

      for (const { normal, vertices, normals, uvs, indices } of Chunk.directions) {
        const neighbor = new Vector3(x, y, z).add(normal)
        const { x: nX, y: nY, z: nZ } = neighbor
        if (!this.isOutsideOfChunk(neighbor) && this.getBlockType(nX, nY, nZ) !== 0) {
          continue
        }
    ...
    this.geometry.setAttribute('position', new BufferAttribute(new Float32Array(allVertices), 3))
    this.geometry.setAttribute('normal', new BufferAttribute(new Float32Array(allNormals), 3))
    this.geometry.setAttribute('uv', new BufferAttribute(new Float32Array(allUvs), 2))
  ...
  /**
   * Iterates all blocks: increments x first, then y, last z. That is the same order as in this.blockTypes.
   * @param {Function} callback with arguments x,y,z and the index of the block in this.blockTypes
   */
  iterateBlocks (callback) {
    let index = 0
    for (let z = 0; z < this.chunkSize.z; z++) {
      for (let y = 0; y < this.chunkSize.y; y++) {
        for (let x = 0; x < this.chunkSize.x; x++) {
          callback(x, y, z, index++)
        }
      }
    }
  }
  ...
  getBlockIndex (x, y, z) {
    return x +
           y * this.chunkSize.x +
           z * this.chunkSize.x * this.chunkSize.y
  }
  ...
  isOutsideOfChunk ({ x, y, z }) {
    return (
      x < 0 || this.chunkSize.x <= x ||
      y < 0 || this.chunkSize.y <= y ||
      z < 0 || this.chunkSize.z <= z
    )
  }
}
